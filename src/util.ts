...
export async function connectMetaMask(expiretAt:number,contractSign: string, proposerContractAddress: string, proposerTokenId: number, account: string, fusion_index: number) {
    try {
        ...
        const chainId = await provider.getNetwork().then((network) => network.chainId);
        const signer = await provider.getSigner();
        const signature = await eip712Signature(await signer, proposerContractAddress, proposerTokenId, expiretAt, Number(chainId));
        const contractGenofusion = new ethers.Contract(contractAddress, contractAbi, signer);
        console.log("contractGenofusion", contractGenofusion)
        const mintParams = {
            proposerContractAddress: proposerContractAddress,
